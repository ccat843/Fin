    ...
    write_json(
        "05_invariant_violations.json",
        {
            "invariant_violations": {
                path: artifact["violations"]
                for path, artifact in run.artifacts.items()
                if isinstance(artifact, dict)
            },
            "vulnerability_hypotheses": [hypothesis.to_dict() for hypothesis in run.hypotheses],
            "hypothesis_validations": [validation.to_dict() for validation in run.hypothesis_validations],
        },
    )
